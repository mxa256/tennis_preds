...
├── api.R                   # Plumber API: serves UI at /, /players, /predict (Elo predictor)
├── index.html              # Frontend for the local app (served by api.R at :8000)
├── docs/                   # GitHub Pages twin: index.html + model.json, inference in-browser
│                           #   model.json regenerated by analysis/train_elo_predictor.R;
│                           #   its JS formula MUST mirror R/predict_elo.R (parity-checked)
└── tennis_preds.Rproj
```
