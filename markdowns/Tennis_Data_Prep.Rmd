---
title: "Tennis Modeling of the ATP Tour: Training Data Preprocessing"
author: "Mona Ascha"
...
#   #Plotting
  win_perc_plot <- ggplot(total_perc_20) +
    geom_col(mapping = aes(x=reorder(Name, -win_perc_round), y=win_perc_round), fill='#0066FF') +
    geom_text(mapping = aes(x=reorder(Name, -win_perc_round), y=win_perc_round, label=win_perc_round), vjust=-0.5) +
    theme(axis.text.x=element_text(angle=60, hjust=1)) +
...
##669900 for grass color
#FF9933 for clay color
#0066FF for hard court color
...
##PLEASE REFER TO DOCUMENTATION FOR INFORMATION REGARDING STATISTICS AND MEANING
...
#Feature Extraction: Calculating rolling averages 
```{r}
...
#  "tourney_date_P_1", #Variables that won't help the model
  "tourney_level_P_1", #Variables that won't help the model
  "draw_size",
  "score_P_1",
  "round_P_1",
  "year_P_1",
...
write.csv(data_ids, "/Users/mona/Dropbox/Desktop/Tennis_Analytics/tennis_preds/data/data_ids.csv", row.names=FALSE)
write.csv(data_train, "/Users/mona/Dropbox/Desktop/Tennis_Analytics/tennis_preds/data/data_train.csv", row.names=FALSE)
