...
data <- data %>% select(c(top20_featimp, identifiers))
...
data <- data %>%
  unnest(cols = c(top20_featimp, identifiers))

#Preserve complete cases
data <- data[complete.cases(data),]

data_ids <- data %>% select(identifiers)
data_train <- data %>% select(top20_featimp)

#Let's export these files for our modeling RMD
write.csv(data_ids, "/Users/mona/Dropbox/Desktop/Tennis_Analytics/tennis_preds/test2023_data_ids.csv", row.names=FALSE)
write.csv(data_train, "/Users/mona/Dropbox/Desktop/Tennis_Analytics/tennis_preds/test2023_data_train.csv", row.names=FALSE)
