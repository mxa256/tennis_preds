...
#For RF: we will keep top 20 features 
#For SVM: the cost function, gamma
#for C in [10, 100, 1000, 2000, 3000, 4000, 5000]
#for g in [0.1, 0.01, 0.001, 0.0001]
#For XGB: eta = 0.1, nrounds = 25, max_depth = 15, subsample = 0.5 (we won't tune all of these)

#Metrics: train_acc, test_acc, tn, fn, fp, tp, f1_score, precision_0, precision_1, recall_0, recall_1, roc_auc
...
#                           tags = NULL)
# with(run, {
#   output = writeLines("blah", "output.txt")    
#   mlflow_log_artifact("output.txt")
#   })
# 
# with(mlflow_start_run(),{
#   #pick experiment id from the pervious step and replace in the next step
#   run1= mlflow_start_run(experiment_id=experiment_id,client=client)
#   #mlflow_log_param("parameter", 5, run_id=run1$run_id, client=client)
#   #mlflow_log_metric("metric", 0, run_id=run1$run_id, client=client)
#   mlflow_log_artifact("output.txt")
# 
# })
```
...
```{r}
#You'll have to import data here 
#You'll have to do preprocessing here
#***INSERT PREPROCESSING HERE

#We'll work with a small sample of data here 
data_sample <- cbind(X_train_rf, Y_train)
data_sample <- data_sample[1:500,]
clean_dataframe <- data_sample

#Data train test split outside of the loop 
...
experiment_name = "Tennis_Preds: XGBoost Sample"
run_name = 'XGBoost_Radial_Kernel_Sample'
description = 'Test run of XGBoost sample n=100'
...
#run_name <- specify if you want to add to a run 
    ...
    with(mlflow_start_run(experiment_id=experiment_id,
                          ...
                          ),
         {
          ...
          #Calibration Curve 
           ...
           #predictions_df <- as.data.frame(attr(predictions, "probabilities"))
           #colnames(predictions_df) <- c("Lose", "Win")
           #predictions_df_win <- predictions_df$Win
           ...
           mlflow_log_param("Params", paste(C, g, seed))
